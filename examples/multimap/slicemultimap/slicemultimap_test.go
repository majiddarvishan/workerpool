...
func AssertMultiMapImplementation(t *testing.T) {
	var _ multimap.MultiMap = slicemultimap.New()
}
...
func benchmarkGet(b *testing.B, m *slicemultimap.MultiMap, size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Get(n)
		}
...
func benchmarkPut(b *testing.B, m *slicemultimap.MultiMap, size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Put(n, struct{}{})
		}
...
func benchmarkPutAll(b *testing.B, m *slicemultimap.MultiMap, size int) {
	v := make([]interface{}, 0)
	v = append(v, struct{}{})
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
...
func benchmarkRemove(b *testing.B, m *slicemultimap.MultiMap, size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Remove(n, struct{}{})
		}
...
func benchmarkRemoveAll(b *testing.B, m *slicemultimap.MultiMap, size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.RemoveAll(n)
		}
